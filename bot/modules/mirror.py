...
class MirrorListener:
    ...
    def onDownloadComplete(self):
        with download_dict_lock:
            LOGGER.info(f"╔—●ℹ️ Download completed: {download_dict[self.uid].name()}")
        ...
        if self.isLeech:
            ...
            LOGGER.info(f"╔—●ℹ️ Leech Name: {up_name}")
            ...
            LOGGER.info(f"╔—●ℹ️ Upload Name: {up_name}")
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        msg = f'<b>╔—●ℹ️ Name: </b><code>{escape(name)}</code>\n<b>╟—●📦Size: </b>{size}'
        if self.isLeech:
            count = len(files)
            msg += f'\n<b>╟—●🗂️Total Files: </b>{count}'
            if typ != 0:
                msg += f'\n<b>╟—●🗃️Corrupted Files: </b>{typ}'
            msg += f'\n<b>╚—●🏷️Uploaded by: </b>{self.tag}\n\n'
            ...
            LOGGER.info(f'╔—●ℹ️ Done Uploading {name}')
