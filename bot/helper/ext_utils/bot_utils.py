...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[START:], start=1):
            msg += f"<b>╔—●ℹ️ Name:</b> <code>{escape(str(download.name()))}</code>"
            ...
            if download.status() not in [
                ...
                msg += f"\n<b>╟—●⚡️Speed:</b> {download.speed()} | <b>⌛️ETA:</b> {download.eta()}"
                try:
        ...
        bmsg += f"\n<b>💾RAM:</b> {virtual_memory().percent}% | <b>⏲️UPTIME:</b> {currentTime}"
