...
class GoogleDriveHelper:
    ...
    def clone(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'<b>╔—●ℹ️ Name: </b><code>{meta.get("name")}</code>\n<b>╟—●📦Size: </b>{get_readable_file_size(self.transferred_size)}'
                msg += '\n<b>╟—●🔖Type: </b>Folder'
                ...
                msg += f'<b>╔—●ℹ️ Name: </b><code>{file.get("name")}</code>'
    ...
    def __create_directory(self, directory_name, parent_id):
        file_metadata = {
            "name": directory_name,
            "description": "Uploaded by Ih-Nasim-telegram-bot",
        ...
        for content in self.telegraph_content:
            self.path.append(
                telegraph.create_page(
                    title='Ih-Nasim-Bot Drive Search',
                    content=content
                )["path"]
            )
        time.sleep(0.5)
    ...
    def count(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'<b>╔—●ℹ️ Name: </b><code>{name}</code>'
